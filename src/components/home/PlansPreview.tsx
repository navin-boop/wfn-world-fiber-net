      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {PLANS.slice(0, 3).map((plan, i) => (
            <div
              key={i}
              className={`plan-card relative rounded-2xl p-8 flex flex-col border transition-shadow hover:shadow-xl ${plan.popular ? 'ring-2' : ''}`}
              style={{
            ...
            >
              ...
              <div className="mb-5">
                ...
                <h3 className="font-bold text-lg mb-2" style={{ color: 'var(--black)' }}>{plan.name}</h3>
                <div className="flex items-baseline gap-1.5 mb-1">
                  <span className="text-5xl font-bold font-mono" style={{ color: 'var(--black)' }}>
                    {plan.speed}
                  </span>
                  <span className="text-base font-semibold" style={{ color: 'var(--gray-500)' }}>
                    {plan.unit}
                  </span>
                </div>
                <p className="text-sm" style={{ color: 'var(--gray-400)' }}>download speed</p>
              </div>

              <div className="mb-7">
                <div className="flex items-baseline gap-1">
                  <span className="text-sm font-medium" style={{ color: 'var(--gray-500)' }}>Rs.</span>
                  <span className="text-4xl font-bold font-mono" style={{ color: plan.popular ? 'var(--green-700)' : 'var(--black)' }}>
                    {plan.price}
                  </span>
                  <span className="text-sm" style={{ color: 'var(--gray-400)' }}>/mo</span>
                </div>
              </div>
        ...
        <div className="text-center mt-12">
          <Btn href="/packages" variant="ghost" size="md">
            View All 5 Plans <ArrowRight size={16} />
          </Btn>
        </div>
      </div>
    </section>
